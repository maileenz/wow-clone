...
export class AuthHelper {
  static MAX_PRE_BC_CLIENT_BUILD = 6141;

  static IsAcceptedClientBuild(build: number) {
    return build <= this.MAX_PRE_BC_CLIENT_BUILD;
  }
  static IsPostBCAcceptedClientBuild(build: number) {
    return build > this.MAX_PRE_BC_CLIENT_BUILD;
  }
  static IsPreBCAcceptedClientBuild(build: number) {
    return true;
  }
}
